...
def render_news(news_data_by_parties): # list, with three dictionary
    return render_template('news.html', news_data=news_data_by_parties)
...
# Schedule the message to be sent every 6 hours
scheduler.add_job(send_daily_news, trigger=CronTrigger(hour='*/6'))
...
def handle_user_input_command(update: Update, context: CallbackContext):
    print("Handling user input command...")
    ...
    if command == "sentiment":
        ...
        if keyword_label == None or average_keyword_sentiment == None or img_buffer == None:
...
def remove_keyword(text, keyword):
    ...
    if keyword in text:
        # Remove the keyword using the replace method
...
def get_keyword_information(input_text):
        ...
        if len(news_data.index) == 0:
            ...
            analyze_data['Tokenized Text'] = analyze_data['Content'].apply(tokenize)
            all_article_text = " ".join(analyze_data['Tokenized Text'])
